...
文档目录      → docs/{创新点.md , 改进位置.md , 指令.md}
工具脚本      → tools/
...
| 判断 | 决定 | 示例 |
|------|------|------|
| 机制与已有模块重叠（如通道注意力） | ❌ 跳过 | CAM → 与 SE/ECA 重叠 |
| 需要 CUDA 扩展 / mmcv-full | ❌ 跳过或改写 | DCNv3 → 需编译 CUDA |
| 完整 backbone/neck 替换，非即插即用 | ❌ 跳过 | Swin-T, GFPN |
| 纯 PyTorch，机制新颖，<1M 参数 | ✅ 纳入 | CoordConv, StarBlock |
| 纯 PyTorch，机制新颖，>1M 但有明确价值 | ⚠️ 需评估 | ICAFusion (16.6M) |
...
**6c. 指令.md** — 更新模块清单表中的计数和条目，确保模块参数速查表同步。
...
### fusion/ (5)
ScalSeq, AFF, iAFF, CIFusion, ICAFusion
