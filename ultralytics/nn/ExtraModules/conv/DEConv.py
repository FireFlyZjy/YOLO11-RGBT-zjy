...
class Conv2d_cd(nn.Module):
    """中心差分卷积 (Central Difference): 强调中心像素差异"""
    ...
    def get_weight(self):
        ...
        conv_weight_cd = torch.zeros(conv_shape[0], conv_shape[1], 9,
...
class Conv2d_ad(nn.Module):
    """角度差分卷积 (Angular Difference): 旋转偏移权重减法"""
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=1, dilation=1, groups=1, bias=False, theta=1.0):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride,
...
class Conv2d_rd(nn.Module):
    """旋转差分卷积 (Rotated Difference): 扩展为5x5后旋转减"""
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
    ...
    def forward(self, x):
        ...
        conv_weight = self.conv.weight
        conv_shape = conv_weight.shape
        conv_weight_rd = torch.zeros(conv_shape[0], conv_shape[1], 25,
                                     dtype=conv_weight.dtype, device=conv_weight.device)
        conv_weight = Rearrange('c_in c_out k1 k2 -> c_in c_out (k1 k2)')(conv_weight)
        ...
        return nn.functional.conv2d(input=x, weight=conv_weight_rd, bias=self.conv.bias,
                                     stride=self.conv.stride, padding=self.conv.padding,
                                     groups=self.conv.groups)


class Conv2d_hd(nn.Module):
    """水平差分卷积 (Horizontal Difference): 左列=+w, 右列=-w"""
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
    ...
    def get_weight(self):
        conv_weight = self.conv.weight
        conv_shape = conv_weight.shape
        conv_weight_hd = torch.zeros(conv_shape[0], conv_shape[1], 9,
                                     dtype=conv_weight.dtype, device=conv_weight.device)
...
class Conv2d_vd(nn.Module):
    """垂直差分卷积 (Vertical Difference): 上行=+w, 下行=-w"""
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=1, dilation=1, groups=1, bias=False):
        super().__init__()
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, stride,
    ...
    def get_weight(self):
        conv_weight = self.conv.weight
        conv_shape = conv_weight.shape
        conv_weight_vd = torch.zeros(conv_shape[0], conv_shape[1], 9,
                                     dtype=conv_weight.dtype, device=conv_weight.device)
...
class DEConv(nn.Module):
    """DEConv: Detail-Enhanced Convolution (细节增强卷积)
    训练时5路并行(中心差分+角度差分+旋转差分+水平差分+垂直差分+标准卷积),
    推理时通过switch_to_deploy()融合为单个3x3卷积,零推理开销
    """
    ...
    def forward(self, x):
        if hasattr(self, 'conv1_1'):
            ...
            w = w1 + w2 + w3 + w4 + w5
            b = b1 + b2 + b3 + b4 + b5
            res = nn.functional.conv2d(input=x, weight=w, bias=b, stride=1, padding=1)
        else:
            res = self.conv1_5(x)
        return self.act(self.bn(res))
